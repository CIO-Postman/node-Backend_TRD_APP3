...
FROM (
	...
	---- JOIN PARCEL_HD ----
	LEFT OUTER JOIN (
	SELECT TMAS.* FROM land.dbo.PARCEL_HD_10 TMAS
	LEFT OUTER JOIN
	( SELECT * FROM (	SELECT COUNT(1) COUNT_HD , BRANCH_CODE , UTM_CODE , UTM_NO_P , UTM_NO , UTM_PAGE , UTM_RATIO , UTM_LANDNO FROM land.dbo.PARCEL_HD_10
	...
	LEFT OUTER JOIN TDASSET_FORMER.tdadmin.PARCEL_REL_47 TREL 
	ON T1.BRANCH_CODE = TREL.BRANCH_CODE
	AND T1.PARCEL_RN = TREL.PARCEL_RN
	LEFT OUTER JOIN 
	...
	AND T1.PARCEL_RN = @PARCEL_RN) TT1
	) TJOIN
...
SELECT   CURR_EVAPRICE, VAL_PER_WAH 
FROM TDASSET_FORMER.tdadmin.PARCEL_47_10
WHERE CHANGWAT_CODE = @CHANGWAT_CODE AND BRANCH_CODE = @BRANCH_CODE AND PARCEL_RN = @PARCEL_RN

-- declare @tablename varchar(100), @SQL varchar(8000), @tablename_hd varchar(100)
-- set @tablename = 'TDASSET_FORMER.tdadmin.PARCEL_47_'+@CHANGWAT_CODE --here you specify the name
-- set @tablename_hd = 'land.dbo.PARCEL_HD_'+@CHANGWAT_CODE --here you specify the name
-- set @SQL ='
-- UPDATE '+@tablename+'
--   SET LOG_PARCEL = NULL ,
--   A_SUBMIT = NULL ,
...
-- 	---- JOIN PARCEL_HD ----
-- 	LEFT OUTER JOIN (
-- 	SELECT TMAS.* FROM '+@tablename_hd+' TMAS
-- 	LEFT OUTER JOIN
-- 	( SELECT * FROM (	SELECT COUNT(1) COUNT_HD , BRANCH_CODE , UTM_CODE , UTM_NO_P , UTM_NO , UTM_PAGE , UTM_RATIO , UTM_LANDNO FROM '+@tablename_hd+'
-- 	GROUP BY BRANCH_CODE , UTM_CODE , UTM_NO_P , UTM_NO , UTM_PAGE , UTM_RATIO , UTM_LANDNO ) T1 
-- 	WHERE COUNT_HD > 1
-- 	 ) TJOIN1
-- 	 ON TMAS.UTM_CODE = TJOIN1.UTM_CODE
...
--         LEFT OUTER JOIN ( SELECT * FROM TDASSET_FORMER.tdadmin.BLOCK_PRICE_47 WHERE BRANCH_CODE = '+@BRANCH_CODE+' AND ISNULL(BLOCK_PRICE_RN,0) <> 0 ) TPRICE 
--         ON T2.BRANCH_CODE = TPRICE.BRANCH_CODE
--         AND T2.BLOCK_PRICE_RN = TPRICE.BLOCK_PRICE_RNlename_hd
