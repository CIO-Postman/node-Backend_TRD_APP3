BEGIN TRAN
    ...
    LEFT OUTER JOIN (
        SELECT BRANCH_CODE , NS3K_RN , DEPTH_GROUP , SUM(ST_VALUE) SUM_ST_VALUE FROM (
        SELECT
        ROW_NUMBER() OVER(PARTITION BY T2.NS3K_RN , T2.DEPTH_GROUP ORDER BY T2.ST_VALUE DESC ) ROW_NUMS ,
        T2.BRANCH_CODE ,
        T2.NS3K_RN ,
        T2.DEPTH_R ,
        ...
        AND T1.NS3K_RN = @NS3K_RN ) T1
        WHERE DEPTH_GROUP > 0 OR ( DEPTH_GROUP = 0 AND ROW_NUMS = 1)
        GROUP BY  BRANCH_CODE , NS3K_RN , DEPTH_GROUP
        ) TJOIN 
    ...
    FROM TDASSET_FORMER.tdadmin.NS3K_REL_47 TMAS
    LEFT OUTER JOIN (
        SELECT OBJECTID , NS3K_RN , DEPTH_R ,
        ST_VALUE ,
        VALUATION_MANUAL_ ,
        CASE 
		 ...
		 ELSE DEPTH_R	
		 END DEPTH_R_ FROM TDASSET_FORMER.tdadmin.NS3K_REL_47
  ...
  UPDATE TDASSET_FORMER.tdadmin.PARCEL_47_NS3K_16
    ...
    LEFT OUTER JOIN (
    SELECT BRANCH_CODE , NS3K_RN , LOWEST_PRICE , SUM(VAL_PER_WAH_)/COUNT(1) AS VAL_PER_WAH_ , MAX(STREET_VALUE) STREET_VALUE ,
    CASE 
    ...
    FROM (
        ...
        END DEPTH_LEVEL
        FROM TDASSET_FORMER.tdadmin.NS3K_REL_47 T1
        WHERE T1.BRANCH_CODE = @BRANCH_CODE
    ...
    GROUP BY BRANCH_CODE , BLOCK_FIX_RN , TABLE_NO , SUB_TABLE_NO , NS3K_RN , LOWEST_PRICE ) TJOIN 
    ON TMAS.BRANCH_CODE = TJOIN.BRANCH_CODE
    AND TMAS.NS3K_RN = TJOIN.NS3K_RN
    WHERE TMAS.BRANCH_CODE = TJOIN.BRANCH_CODE
    AND TMAS.NS3K_RN = TJOIN.NS3K_RN
