UPDATE TDASSET_FORMER.tdadmin.BLOCK_BLUE_47 
    ...
    SELECT DENSE_RANK () OVER (PARTITION BY T3.BRANCH_CODE , T1.BLOCK_BLUE_RN ORDER BY T3.VAL_PER_WAH DESC ) ROWNUM , T3.BRANCH_CODE , T1.BLOCK_BLUE_RN , T3.NS3K_RN , T3.VAL_PER_WAH FROM 
    (
        ...
        LEFT OUTER JOIN 
        (
            SELECT * FROM TDASSET_FORMER.tdadmin.NS3K_REL_47 
    ...
    SELECT DENSE_RANK () OVER (PARTITION BY T3.BRANCH_CODE  , T1.BLOCK_BLUE_RN ORDER BY T3.VAL_PER_WAH DESC ) ROWNUM , T1.BRANCH_CODE , T1.BLOCK_BLUE_RN , T2.NS3K_RN , T3.VAL_PER_WAH  , 
    ...
    ON T2.BRANCH_CODE = T3.BRANCH_CODE
    AND T2.NS3K_RN = T3.NS3K_RN ) TJOIN
    ON TMAS.BRANCH_CODE = TJOIN.BRANCH_CODE
