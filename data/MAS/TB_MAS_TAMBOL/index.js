...
const ins = async (data) => {
    try {
        ...
        const sqlQueries = await utils.loadSqlQueries('/MAS/TB_MAS_TAMBOL');
...
const selByPK = async(arg) => {
    try {
        ...
        const sqlQueries = await utils.loadSqlQueries('MAS/TB_MAS_TAMBOL');
        const result = await pool.request()
            .input('AMPHUR_CODE', sql.Numeric(5,0), arg)
...
const selSeqByUTM = async (data) => {
    try {
        let pool = await sql.connect(config.sql);
        const sqlQueries = await utils.loadSqlQueries('MAS/TB_MAS_TAMBOL');
        const result = await pool.request()
        .input('BRANCH', sql.NVarChar(15), data.BRANCH)
        .input('AMPHUR_CODE', sql.NVarChar(15), data.AMPHUR_CODE)
        .query(sqlQueries.selSeqByUTM);                            
        return result.recordset;
    } catch (error) {
        return error.message;
    }
